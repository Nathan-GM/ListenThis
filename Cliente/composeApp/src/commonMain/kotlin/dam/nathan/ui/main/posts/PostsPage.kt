...
fun PostsPage(
    ...
    var posts by remember { mutableStateOf(mutableListOf<Post>(Post())) }
    ...
    Scaffold(
        topBar = {
            ...
            BottomAppBar {
                if (genre != null) {
                    Text("Género en busqueda: ${genre.name}")
                }
            }
        },
            ...
            if (firstTry) {
                ...
                scope.launch {
                    ...
                    if (tmp == null) {
                        ...
                        if (genre != null) {
                            postsSize = posts.count { x -> x.genre == genre.id }
                            postByGenre = posts.filter { x -> x.genre == genre.id }.toMutableList()
                            for (post in postByGenre) {
                                println("${post.content} - ${post.genre} ")
                            }
                        } else {
                            postsSize = tmp.size
                        }
                ...
                if (postsSize > 0) {
                    LazyVerticalGrid(
                        ...
                        items(postsSize) { post ->
                            if (genre != null) {
                                if (postByGenre[post].genre == genre.id) {
                                    PostCard(
                                        userVM = userVM,
                                        post = postByGenre[post],
                                        genre = genre,
                                        goToPostDetail = { post, userVM -> println(post) }
                                    )
                                }
                            } else {
